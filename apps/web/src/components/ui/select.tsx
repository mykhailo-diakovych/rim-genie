...
function SelectTrigger({
  ...
  return (
    <SelectPrimitive.Trigger
      className={cn(
        "flex h-9 w-full items-center justify-between rounded-md border bg-white px-2 font-rubik text-sm leading-5 transition-colors outline-none",
    ...
    <SelectPrimitive.Portal>
      <SelectPrimitive.Positioner className="z-50 outline-none">
        <SelectPrimitive.Popup
          className={cn(
            "max-h-60 w-max max-w-[--available-width] min-w-[--anchor-width] origin-[--transform-origin] overflow-y-auto rounded-md border border-field-line bg-white py-1 shadow-[0px_4px_16px_0px_rgba(42,44,45,0.12)] outline-none",
...
function SelectOption({ className, children, ...props }: SelectPrimitive.Item.Props) {
  return (
    <SelectPrimitive.Item
      className={cn(
        "flex cursor-default items-center justify-between px-3 py-2 font-rubik text-sm leading-5 text-body capitalize outline-none select-none",
